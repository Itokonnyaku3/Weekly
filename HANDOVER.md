...
- **★ノート上下キーの選択行遷移が「がたつく」（2026-05-31 米山さん追加・未着手・要原因調査）**: デイリーノートで ↑/↓ を押した時の選択行移動が重い/カクつく。**仮説**: `olKeyDown` の ArrowUp/Down が毎回 `olRender`（ノート全体DOMの再構築）＋フォーカス復元を走らせており、ノードが多いと1キーごとに全再描画でジャンク。**調査/対策方針**: 上下移動は全再描画せず、隣接行要素へ caret/focus を移すだけ（DOM据え置き）に。`_olFocusId` 更新→`olRender` の経路を、移動時は軽量パス（`focus()`＋`_olReapplyIncSearch`不要化）へ。再描画が必要な場合のみ限定。実測（DevTools Performance）で要因特定してから最小修正。
