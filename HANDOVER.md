...
最終更新: 2026-06-02 / 担当バージョン: v1.16.0-06020940-rich-fmt
...
| v1.16.0 | **修飾・貼り付け(課題#2)**: (a)Ctrl+B/色ボタンで選択範囲があればexecCommand(bold/foreColor)でインライン適用→node.html保存、選択なしは従来のnode.bold/colorフラグ (b)Enterの分割をRange HTML分割に→<br>/インライン書式を保持 (c)olInputのrender()を選択中は保留→貼付後の選択消失防止。ロジック実測検証済 |
...
### 進行中バッチ（2026-05-30 米山さん依頼の6課題）★全完了
v1.6.0 **課題1・2・5（フォーカス系）** → v1.7.0 **課題3（視認性・案C）** → v1.7.1 **割り込みバグ（projTag孤立）修正** → v1.8.0 **課題6（gridWk）** → v1.13.0 **課題4（日次バックアップ）**。**原6課題はすべて完了**。
- 追加課題（左セル横スクロール=v1.11.1完了 / コピペ=v1.9.0-1.11.0完了 / 検索統合=v1.12.0完了）。残りは下記「残課題」のバックログ（ミラーリング・修飾貼付・Ctrl+.メニュー・テンプレート 等）。
- **追加課題（コピペ/複数選択の同等化）**: **Phase1完了=v1.9.0**（マーカー方式・Ctrl+A/単体C/X・ハンドラ統合・parentId保持）、**Phase2完了=v1.10.0**（リッチHTML貼付・サニタイズ・入れ子/見出し/装飾保持）、**Phase3完了=v1.11.0**（TSV→表ノード・HTML表取込・表のコピー出力TSV/table）。**Phase4は見送り**（外部HTML内画像のみ未対応＝CORS取得困難。自アプリ往復/コピー出力/スクショ貼付は動作）。**＝コピペ機能は完了**。**仕様書: `仕様書_コピペ貼付機能.md`**。

### 残課題（次セッションで検討するもの）
- **★ミラーリング機能（Workflowy のミラー参照・2026-05-31 米山さん追加・未着手）**:
  - ノードへの「リンク」ではなく、**そのノード配下の階層（サブツリー）をミラー先にそのまま表示**する。
  - **双方向同期**: 元ノード／ミラー先のどちらを編集しても、もう一方に反映。
  - ミラーした側・された側の両ノードに**アイコンを付与**。クリックで**各ミラーの所在へのリンクをポップアップ表示**。
  - ポップアップに**ミラー個別削除ボタン**。押すと**そのミラーだけ**を削除（他のミラー・本体は残す）。
  - ⚠️ **名称衝突注意**: 既存の「ミラーアイテム」(`getMirrorItems` / `.eitem-mirror` / `isMirror`) は *過去週の未完を今週グリッドに再表示する継続機能* で**全くの別物**。このWorkflowy型ミラーは別名（例: `nodeMirror` / `transclusion`）にすること。
  - 設計メモ（着手時の論点）: 同期の実体をどう持つか（共有ノードID参照 vs 実体複製＋双方向反映）、循環ミラー防止、子孫の追加/削除/インデント変更の伝播、グリッド表示（projTag/週）との整合、保存データ構造への影響。要ブレスト＋仕様化。
- **★修飾・貼り付け関連（2026-05-31 米山さん追加・未着手）**:
  - **修飾が選択範囲でなく行単位**: 太字等が `node.bold`（ノード全体フラグ）として効いており、選択した範囲だけに適用できない。選択範囲へのインライン修飾（`execCommand` 系で `node.html` に保持）に作り替える必要。
  - **Shift+Enter の行内改行が Enter 確定後に反映されないように見える**: 行内 `<br>`（node.html）の保存/再描画（`olRichSave`/`olRender`）で改行が落ちている可能性。要調査。
  - **貼り付けた改行・修飾入りテキストの一部選択が消える**: 複数行/装飾を含むノードで範囲選択中に再描画等で選択が解除されるケース。`olRender` の再描画タイミングや contenteditable の選択保持に起因と推測。
- **★Ctrl+. メニュー（スラッシュメニュー）関連（2026-05-31 追加・未着手）**:
  - **「ToDoに変更」⇔「ドットに戻す」をトグル統合**（現状は別項目）。同様に **「リンクに変換」⇔「リンクを解除」もトグル統合**。
  - **IMEオン時のショートカット文字漏れ**: IME変換中にメニューのショートカット（T/D/B 等の単キー）が発火するが、その後入力したキーが文字として入ってしまう。`ev.isComposing` / keyCode 229 でガード、または確定前のキーを抑止する対応が必要。
- **★テンプレート機能（2026-05-31 追加・未着手・要設計）**: よく使う構造を雛形として挿入。例: 会議テンプレ（参加者・議題・決定事項 等の項目を最初から展開）。テンプレ定義の持ち方（専用ノート/設定）、プレースホルダ、挿入UI（Ctrl+.メニュー or 専用）を要ブレスト＋仕様化。
- ~~**ノート上下キーの選択行遷移が「がたつく」/数行飛ぶ**~~ → **v1.15.0 で解消**（原因＝平↑↓が1キーごと `olRender` 全再構築。`olFocusNodeEl` で再描画せず隣接行へ focus+caret 移動の軽量化）。
