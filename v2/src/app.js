...
const { renderDaily, focusCard, resetZoom, clearDayFocus, setZoom, getZoom, getDayFocus, setDayFocus, setMentionJump, setEmbedOpener, setImageLoader, clearSelection, serializeEditable, caretOffset } = await import('./daily.js' + _q);
...
function renderAll(){
  ...
  const ev = document.getElementById('view-embed');
  app?.classList.toggle('split', splitOn);
  if (splitOn){
    // 左=リスト固定 / 右=デイリー・プロジェクト・埋め込み（splitRight）。両ペインを毎回再描画＝片側の変更がもう片側へ反映
    if (lv) lv.hidden = false;
    if (dv) dv.hidden = splitRight !== 'daily';
    if (pv) pv.hidden = splitRight !== 'project';
    if (ev) ev.hidden = splitRight !== 'embed';
    if (lv) renderList(store, lv, renderAll, listState, zoomToCard, openProject);
    if (splitRight === 'embed' && ev) renderEmbed(ev, embedUrl);
    else if (splitRight === 'project' && pv) renderProjectView(store, pv, renderAll, projState, jumpToCard);
    ...
    if (ev) ev.hidden = true;                    // 埋め込みは分割時のみ
...
let embedUrl = '';                        // 分割右ペインの埋め込みURL（セッション）
function openEmbed(url){                   // カードの埋め込みを分割右ペインで開く
  if (!url) return;
  navPush();
  embedUrl = url; splitRight = 'embed'; splitOn = true;
  renderAll();
}
function renderEmbed(cont, url){           // 埋め込みペイン: 上部バー（開く/再読込/閉じる/URL）＋iframe。拒否時もリンクで開ける
  cont.innerHTML = '';
  if (!url){ const e = document.createElement('div'); e.className = 'embed-empty'; e.textContent = 'カードの ⋯ メニューで「埋め込みURL」を設定し「⊞ 埋め込みを開く」で表示します。'; cont.appendChild(e); return; }
  const bar = document.createElement('div'); bar.className = 'embed-bar';
  const open = document.createElement('a'); open.className = 'btn'; open.textContent = '↗ 新しいタブで開く'; open.href = url; open.target = '_blank'; open.rel = 'noopener noreferrer';
  const reload = document.createElement('button'); reload.className = 'btn'; reload.type = 'button'; reload.textContent = '🔄'; reload.title = '再読込';
  const close = document.createElement('button'); close.className = 'btn'; close.type = 'button'; close.textContent = '✕'; close.title = '閉じる'; close.onclick = () => { splitRight = 'daily'; renderAll(); };
  const u = document.createElement('span'); u.className = 'embed-url'; u.textContent = url; u.title = url;
  const frame = document.createElement('iframe'); frame.className = 'embed-frame'; frame.src = url; frame.referrerPolicy = 'no-referrer-when-downgrade';
  reload.onclick = () => { frame.src = frame.src; };
  bar.append(open, reload, close, u);
  cont.append(bar, frame);
}
...
function boot(){
  ...
  setEmbedOpener(openEmbed);                      // カードの埋め込みを分割右ペインで開く
