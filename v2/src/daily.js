...
let _openEmbed = null;      // カードの埋め込みを分割右ペインで開く（app から設定）
...
function buildCardMenu(store, ref, body, requestRender){
  ...
  // 埋め込み（SharePoint上のExcel等を分割右ペインに iframe 表示）
  const emb = document.createElement('input'); emb.type = 'url'; emb.className = 'cm-url'; emb.placeholder = 'https://…(embedview 等)'; emb.value = body.embed || ''; emb.dataset.mk = 'embed';
  emb.onchange = () => { store.updateBody(body.id, { embed: emb.value.trim() || undefined }); requestRender(); refocusMenu('embed'); };
  menu.appendChild(cmField('埋め込みURL', emb));
  const embOpen = document.createElement('button');
  embOpen.type = 'button'; embOpen.className = 'cm-btn'; embOpen.dataset.mk = 'embopen'; embOpen.textContent = '⊞ 埋め込みを開く';
  embOpen.disabled = !(body.embed && body.embed.trim());
  embOpen.onclick = () => { const u = (store.getBody(body.id).embed || '').trim(); if (u && _openEmbed){ _openEmbed(u); } };
  menu.appendChild(embOpen);
...
export function setEmbedOpener(fn){ _openEmbed = fn; }      // カードの埋め込みを分割右ペインで開く（app から設定）
