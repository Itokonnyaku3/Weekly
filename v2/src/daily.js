...
const _collapsedDays = new Set();   // 明示的に折りたたんだ日付（セッション内）
const _expandedDays = new Set();    // 明示的に展開した「既定は折りたたみ」の古い日（セッション内）
let _dayLimit = 0;                  // 「全体」ビューで描画する日セクションの上限（0=未初期化→DAY_PAGE）
let _mentionJump = null;    // @チップのクリック→移動（app から設定）
let _openSavedSearch = null;   // ⟦s:id⟧ チップ→保存検索を開く（app から設定）
...
// ── 「全体」ビューの描画量を有界化する窓（データ増加対策）──
// 過去データを一度に全部読むと日数に比例して DOM が膨張するため、
//  ・直近 RECENT_EXPANDED 日ぶんの日セクションは既定で展開、それより古い日は既定で折りたたみ（見出しだけ＝軽量）
//  ・日セクションは DAY_PAGE 件までしか描画せず、「さらに読み込む」で過去を追記
// 折りたたみ/展開はユーザーが日ごとに上書き可能（_collapsedDays / _expandedDays）。
const RECENT_EXPANDED = 14;   // 直近この日数ぶんの日セクションは既定で展開
const DAY_PAGE = 60;          // 一度に描画する日セクション数（「さらに読み込む」で +DAY_PAGE）

// 日付の折りたたみ状態（明示操作を優先、既定は oldDefault＝窓の外の古い日を折りたたみ）
function isDayCollapsed(date, oldDefault){
  if (_expandedDays.has(date)) return false;
  if (_collapsedDays.has(date)) return true;
  return !!oldDefault;
}
// 明示操作を記録（両 Set を排他に保ち、既定に依らず状態を確定させる）
function setDayCollapsed(date, collapsed){
  if (collapsed){ _expandedDays.delete(date); _collapsedDays.add(date); }
  else { _collapsedDays.delete(date); _expandedDays.add(date); }
}
...
export function renderDaily(store, mount, requestRender, onMentionJump){
  ...
  if (_focusDate){
    ...
    if (day){
      ...
      mount.appendChild(renderDaySection(store, day, requestRender, false, false, true));  // 単日フォーカスは常に展開
  ...
  // 窓表示: 直近は展開・古い日は既定で折りたたみ。描画は _dayLimit 件までで、残りは「さらに読み込む」。
  if (!_dayLimit) _dayLimit = DAY_PAGE;
  const shown = days.slice(0, _dayLimit);
  for (let i = 0; i < shown.length; i++){
    const oldDefault = i >= RECENT_EXPANDED;    // 直近 RECENT_EXPANDED 件より古い＝既定で折りたたみ
    mount.appendChild(renderDaySection(store, shown[i], requestRender, true, oldDefault, false));
  }
  if (days.length > _dayLimit){
    const more = document.createElement('button');
    more.className = 'day-load-more'; more.type = 'button';
    more.textContent = `さらに読み込む（残り ${days.length - _dayLimit} 日）`;
    more.onclick = () => { _dayLimit += DAY_PAGE; requestRender(); };
    mount.appendChild(more);
  }
  manageOutsideClose(requestRender);   // メニューが開いていれば外側クリックで閉じる
}

function toggleDay(date, requestRender, oldDefault){    // 日別の折りたたみトグル（セッション内・既定を考慮）
  setDayCollapsed(date, !isDayCollapsed(date, oldDefault));
...
function renderDaySection(store, day, requestRender, focusable, oldDefault, forceExpand){
  const dayRef = store.refsForBody(day.id).find(r => r.parentRefId === null);
  const collapsed = forceExpand ? false : isDayCollapsed(day.content, oldDefault);
  ...
  tog.onclick = (e) => { e.stopPropagation(); toggleDay(day.content, requestRender, oldDefault); };
  ...
  if (focusable){
    ...
    head.onclick = () => { setDayCollapsed(day.content, false); _focusRef = null; _focusDate = day.content; requestRender(); focusDayHead(day.content); };
  }
  sec.appendChild(head);
  if (dayRef && !collapsed){
...
function saveTable(store, body, rows, widths, headerColor){
  const o = { rows };
  if (widths && widths.length) o.widths = widths;
  if (headerColor) o.headerColor = headerColor;
  store.updateBody(body.id, { content: JSON.stringify(o) });
}
...
const ROWDEL_W = 22;
// 表の右クリックメニュー（単一・body直下に配置）
let _tblMenuEl = null, _tblMenuCloser = null, _tblMenuKey = null;
function closeTableMenu(){
  if (_tblMenuEl){ _tblMenuEl.remove(); _tblMenuEl = null; }
  if (_tblMenuCloser){ document.removeEventListener('mousedown', _tblMenuCloser); _tblMenuCloser = null; }
  if (_tblMenuKey){ document.removeEventListener('keydown', _tblMenuKey, true); _tblMenuKey = null; }
}
function buildTableWidget(store, ref, body, requestRender){
  ...
  let headerColor = ''; try { const o = JSON.parse(body.content || '{}'); if (typeof o.headerColor === 'string') headerColor = o.headerColor; } catch (_){}
  const save = () => saveTable(store, body, rows, widths, headerColor);   // headerColor を保ったまま保存
  ...
  rows[0].forEach((_, c) => {
    ...
    b.onclick = () => { if (rows[0].length > 1){ rows.forEach(rw => rw.splice(c, 1)); widths.splice(c, 1); save(); requestRender(); } };
  ...
  rows.forEach((row, r) => {
    ...
    delBtn.onclick = () => { if (rows.length > 1){ rows.splice(r, 1); save(); requestRender(); } };
    ...
    row.forEach((cell, c) => {
      ...
      if (r === 0 && headerColor) td.style.background = headerColor;   // ヘッダー色
      td.addEventListener('input', () => { rows[r][c] = td.textContent; save(); });
      td.addEventListener('contextmenu', (e) => { e.preventDefault(); openTableMenu(e.clientX, e.clientY, r, c); });   // 右クリックで行/列メニュー
      td.addEventListener('keydown', (e) => {                // Tab=次のセル（最終で新行）／Shift+Tab=前のセル／Alt+Shift+↑↓=行移動
        if (e.altKey && e.shiftKey && (e.key === 'ArrowUp' || e.key === 'ArrowDown')){   // 記入済み行の上下移動（ヘッダは動かさない）
          e.preventDefault();
          const nr = e.key === 'ArrowUp' ? r - 1 : r + 1;
          if (r >= 1 && nr >= 1 && nr < rows.length){
            const t = rows[r]; rows[r] = rows[nr]; rows[nr] = t; save(); requestRender();
            const cells = (_ctx.container || document).querySelectorAll(`[data-ref="${ref.id}"] .ct-cell`);
            caretToCell(cells[nr * ncol + c]);
          }
          return;
        }
        if (e.key !== 'Tab') return;
        ...
        rows.push(new Array(ncol).fill('')); save(); requestRender();   // 最終セルで Tab → 行追加
        const cells = (_ctx.container || document).querySelectorAll(`[data-ref="${ref.id}"] .ct-cell`);
      ...
      if (r === 0){                                          // 列幅リサイズ（ヘッダ右端をドラッグ）
        ...
        rz.addEventListener('pointerdown', (e) => {
          ...
          const onUp = () => { rz.removeEventListener('pointermove', onMove); rz.removeEventListener('pointerup', onUp); save(); };
  ...
  addRow.onclick = () => { rows.push(new Array(ncol).fill('')); save(); requestRender(); };
  const addCol = document.createElement('button'); addCol.type = 'button'; addCol.className = 'ct-btn'; addCol.textContent = '＋列';
  addCol.onclick = () => { rows.forEach(rw => rw.push('')); widths.push(120); save(); requestRender(); };
  ...
  // 右クリックメニュー（行/列の挿入・移動・削除・ヘッダー色）。r,c はクリックしたセルの行・列。
  function openTableMenu(clientX, clientY, r, c){
    closeTableMenu();
    const m = document.createElement('div'); m.className = 'ct-menu'; m.contentEditable = 'false';
    const item = (label, fn, extra) => {
      const b = document.createElement('button'); b.type = 'button'; b.className = 'ct-menu-item' + (extra ? ' ' + extra : '');
      b.textContent = label;
      b.onclick = (e) => { e.preventDefault(); e.stopPropagation(); closeTableMenu(); fn(); };
      m.appendChild(b); return b;
    };
    const sep = () => { const s = document.createElement('div'); s.className = 'ct-menu-sep'; m.appendChild(s); };
    item('↑ 上に行を追加', () => { rows.splice(r, 0, new Array(ncol).fill('')); save(); requestRender(); });
    item('↓ 下に行を追加', () => { rows.splice(r + 1, 0, new Array(ncol).fill('')); save(); requestRender(); });
    item('↑ 行を上へ移動', () => { if (r >= 1 && r - 1 >= 1){ const t = rows[r]; rows[r] = rows[r - 1]; rows[r - 1] = t; save(); requestRender(); } });
    item('↓ 行を下へ移動', () => { if (r >= 1 && r + 1 < rows.length){ const t = rows[r]; rows[r] = rows[r + 1]; rows[r + 1] = t; save(); requestRender(); } });
    item('✕ この行を削除', () => { if (rows.length > 1){ rows.splice(r, 1); save(); requestRender(); } }, 'ct-menu-del');
    sep();
    item('← 左に列を追加', () => { rows.forEach(rw => rw.splice(c, 0, '')); widths.splice(c, 0, 120); save(); requestRender(); });
    item('→ 右に列を追加', () => { rows.forEach(rw => rw.splice(c + 1, 0, '')); widths.splice(c + 1, 0, 120); save(); requestRender(); });
    item('✕ この列を削除', () => { if (ncol > 1){ rows.forEach(rw => rw.splice(c, 1)); widths.splice(c, 1); save(); requestRender(); } }, 'ct-menu-del');
    sep();
    const colorRow = document.createElement('label'); colorRow.className = 'ct-menu-color';
    colorRow.appendChild(document.createTextNode('ヘッダー色 '));
    const ci = document.createElement('input'); ci.type = 'color'; ci.value = /^#[0-9a-f]{6}$/i.test(headerColor) ? headerColor : '#eef1f6';
    ci.addEventListener('change', () => { headerColor = ci.value; closeTableMenu(); save(); requestRender(); });
    colorRow.appendChild(ci);
    m.appendChild(colorRow);
    document.body.appendChild(m); _tblMenuEl = m;
    const mw = m.offsetWidth, mh = m.offsetHeight;
    m.style.left = Math.max(4, Math.min(clientX, window.innerWidth - mw - 6)) + 'px';
    m.style.top = Math.max(4, Math.min(clientY, window.innerHeight - mh - 6)) + 'px';
    _tblMenuCloser = (e) => { if (!e.target.closest('.ct-menu')) closeTableMenu(); };
    _tblMenuKey = (e) => { if (e.key === 'Escape'){ e.preventDefault(); closeTableMenu(); } };
    setTimeout(() => { document.addEventListener('mousedown', _tblMenuCloser); document.addEventListener('keydown', _tblMenuKey, true); }, 0);
  }
  return wrap;
}
...
function onDayHeadKey(e, store, day, requestRender){
  ...
  if ((e.ctrlKey || e.metaKey) && !e.shiftKey && (e.key === 'ArrowUp' || e.key === 'ArrowDown')){   // Ctrl+↑ 折りたたみ / Ctrl+↓ 展開
    e.preventDefault();
    setDayCollapsed(day.content, e.key === 'ArrowUp');   // Ctrl+↑=折りたたみ / Ctrl+↓=展開（既定を上書き）
...
// 「全体」ビューの窓表示で、指定日を確実に見える状態にする（ジャンプ/カレンダー用）。
// 既定で折りたたまれる古い日を展開扱いにし、描画上限(_dayLimit)をその日の位置まで広げる＝ジャンプ先が必ず DOM に出る。
export function revealDay(store, date){
  if (!date) return;
  setDayCollapsed(date, false);
  const days = store.queryBodies(b => b.kind === 'day').map(b => b.content)
    .sort((a, b) => (a < b ? 1 : a > b ? -1 : 0));   // 新しい日付が上（render と同順）
  const idx = days.indexOf(date);
  if (!_dayLimit) _dayLimit = DAY_PAGE;
  if (idx >= 0 && idx + 1 > _dayLimit) _dayLimit = idx + 1;   // その日を含む位置まで描画
}
