...
## v1.22.1-06111200-agenda-todo-only (2026-06-11)
### 修正 — アジェンダはタスク(todo)のみ表示

- デイリーアジェンダ（期限超過/本日期限/本日作業期間/期限なし）の全セクションを **`getNodeType(n)==='todo'` のみ** に限定。メモ/ログ・リンク・ノードリンク等は `due`/スパンを持っていても対象外に。
- 実装: `getDayAgenda` のフィルタを「todoのみ」に統一（従来は due 付きログ等も期限超過/本日期限に出ていた）

---

## v1.22.0-06111100-agenda-overdue-nodue (2026-06-11)
### 機能追加 — デイリーアジェンダに「期限超過」「期限なし」を追加

v1.21.0 の本日期限に加え、2セクションを追加。

- **⚠ 期限超過（〜昨日）**: `due < 本日` の未完タスク（元の期限を赤チップで表示）
- **期限なし**: `due` 未設定の未完**todo**（ログ等の無期限ノードは除外）。**既定で折りたたみ**（`▶ 期限なし (N)`、クリックで展開）。展開時は上限60件＋「他M件」
- **期限超過・期限なしは「今日」のデイリーノートのみ**表示（過去/未来の日付では従来通り本日期限・本日作業期間のみ）
- 表示順: ⚠期限超過 → 本日期限 → 本日作業期間 → 期限なし(折りたたみ)
- 期限超過・期限なしのtodoも**チェックで完了**（その場でトグル→グリッド反映）
- 実装: `getDayAgenda` を `{overdue,due,span,nodue}` に拡張、`_agNoDueExpanded` 折りたたみ状態、`olAgendaToggleNoDue()`、`renderKey` の agenda 署名を拡張

---

## v1.21.0-06111000-daily-agenda (2026-06-11)
### 機能追加 — デイリーノートに「本日期限 / 本日作業期間」自動セクション

デイリーノートの最下部に、表示中の日付を基準としたアジェンダを自動表示。

- **本日期限**: 全ノード横断（全プロジェクト＋未割当）で `due` が表示日と一致する**未完**タスク
- **本日作業期間**: `startDate ≤ 表示日 ≤ endDate`（スパン内）の**未完**タスク。範囲チップ `📊MM-DD〜MM-DD` 付き
- **基準日は表示中の日付**（★今日に限定せず、任意の日のノートでその日のアジェンダ）
- **本日期限のチェックボックスは機能**: その場で完了トグル→元ノード＆グリッドに反映。未完のみ表示なので完了すると一覧から消える
- 各行クリックで**元ノードへジャンプ**
- `proj:` ノートでは非表示。両セクション0件なら区切り線ごと非表示
- 非編集の参照行（searchsummary結果と同じ仕組み）。既存ノード編集・既存関数には非干渉
- 実装: `getDayAgenda(date)` / `olAgendaRowHtml()` / `olAgendaToggle()`、olRender 末尾に追記、`renderKey` に agenda 署名 `_agKey` を追加して due/span/完了変更に追従。日付はゼロ埋め正規化で比較

---

## v1.20.0-06061500-note-props-panel (2026-06-06)
### 機能追加 — ノート側からプロパティ編集（方式1：フルパネル）

これまでグリッドの Shift+Enter でしか開けなかった詳細パネルを、**ノート（アウトライン）からも開けるように**。グリッドに出ない未割当ノードも編集可能に。

- スラッシュメニューに **「⚙ プロパティ（期限・優先度・メモ…）」**（キー `E`）を追加 → グリッドと同じ詳細パネルを開く
- `olOpenProps(date, nodeId)`: ノードの `projTag→pi`・`日付(gridWk優先)→wk` を解決して `openPanel` を呼ぶ
- **未割当（projTagなし）ノードにも対応**: `buildPB`・`savePanel` の `S.projects[pi]` 参照を pi<0 でガード（従来は例外）。未割当時は「（未割当）」表示・「次の週に延期」非表示
- 保存/キャンセル後は**グリッドではなくノートの元ノードへフォーカスを戻す**（`_panelFromNote`）
- 全画面フォーカスモード（note-panel `z-index:999`）でもパネルが隠れないよう、ノート起点時は `#panel.panel-overlay`（`position:fixed; z-index:1000`）で前面表示
- グリッド起点の従来動作は不変（`openPanel` 冒頭で `_panelFromNote=null`）

---

## v1.19.0-06061400-inline-meta-chips (2026-06-06)
### 機能追加 — ノート行に期限/優先度のインラインチップ（案A Phase1）

詳細パネル（Shift+Enter）にしか出ていなかったメタ情報を、ノート（アウトライン）行にインライン表示。「詳細がパネルに隠れて見えない＝グリッドとノートの心理的分離」への対処の第一歩。

- ノート行のテキスト右に **期限📅 と 優先度⚑ のチップ**を表示（**設定済みのものだけ**＝プログレッシブ・ディスクロージャ）
- 期限チップは近さで色分け：超過=赤 / 当日=金 / 先=淡色 / 完了=取消線（`getDueBadge` と色判定を統一）
- 優先度チップ：高=赤 / 中=金 / 低=青
- **表示のみ**（編集は引き続き詳細パネル）。Phase2 でクリック編集・スラッシュ追加・`!`構文を予定
- グリッド側は既存の期限バッジ＋優先度バーで対応済みのため**無変更**（デグレ最小）
- 実装: `olMetaChips(n)` ヘルパー新設、olRender 行テンプレートに注入、renderKey に `due/priority` を追加（パネル編集後にチップが即更新）

---

## v1.18.0-06061300-unassigned-row (2026-06-06)
### 機能追加 — 未分類タスク仮想行（課題#2）

プロジェクトに紐づいていない未完了ToDoが「行方不明」になる問題への対処。

- グリッド**最上部に警告色（アンバー）の「⚠ 未分類タスク」行**を常設
- 各週セルに、その週の **未割当 × 未完了ToDo を実タスクとして列挙**（件数だけでなく中身を表示）
- タスクをクリック → 該当ノートを開く（そこで既存の `@` メニューでプロジェクト割当）。仮想行自体は**表示のみ**（割当UIは持たない）
- 表示中の全週で未割当0件のときは**行ごと非表示**
- リンク列に総件数バッジを表示
- 実効週は `gridWk` 優先（`getGridItems` と同一ロジック）
- 実装: `getUntaggedOpenTodos(wk)` ヘルパー新設。`_renderImpl` のプロジェクト描画ループ手前に**独立ブロックを追加**（既存ループ・`getGridItems`等は無変更でデグレ回避）

---

## v1.17.0-06061230-search-filters (2026-06-06)
### 機能追加 — 検索の絞り込みフィルタ（課題#3）

検索モーダルに「種別 / 完了状態 / プロジェクト紐づけ」のフィルタ行を追加。

- **種別**: すべて / ☐ToDo / 🔗リンク / 📝ログ（`getNodeType` ベース）
- **完了状態**: すべて / 未完了 / 完了
- **紐づけ**: すべて / PJ付き / 未割当
- 各グループは単一選択（既定 `all`）、クリックで即再検索
- **フィルタ単独検索を許可**: テキスト・タグが無くてもフィルタだけで検索可能（例「未割当の未完了ToDoを全件」）→ 課題#2の仕分けに直結
- 保存検索（searchsummary）も `savedQuery.filters` にフィルタを保存・復元。**未指定時は全て `all` 扱いで既存の保存ノードと後方互換**
- フィルタ適用時はプロジェクト名のヒット（PROJECT結果）を除外（ノード属性フィルタと無関係なため）
- 実装: `_searchFilters` 状態、`_SEARCH_FILTER_DEFS`、`_searchUpdateFilterRow()`、`_searchSetFilter()`、`_runSearchQuery`/`doSearch` にフィルタ引数を追加

---

## v1.16.3-06061130-fix-searchsave (2026-06-06)
### バグ修正 — 「検索を保存」ボタンが機能しない

- `_searchSave()` が変数 `q2`（第2検索語）を定義せずに参照しており、`ReferenceError` で関数が即中断していた（ボタンを押しても何も起きない）
- `const q2 = ($('search-input2').value || '').trim();` を追加して修正
- これで第2検索語を含む検索条件も searchsummary ノードとして正しく保存される
...
#### 原因
ノートの ↑/↓ 行ナビ（`olKeyDown` の ArrowUp/Down）が**1キーごとに `olRender('ol-container', …)`＝ノート全体DOMの再構築＋フォーカス復元＋ハイライト再適用**を実行していた。ノードが多い/表・画像を含むと1キーが重く、キーリピート時は描画が追いつかずフレーム落ち→フォーカスが数行飛ぶ。

#### 修正
- 新規 `olFocusNodeEl(targetId)`: **全再描画せず、既存の隣接行 `.ol-text` 要素へ `focus()`＋caret配置**するだけの軽量移動。`.ol-text` の `onfocus` が `_olFocusId`・選択解除・書式更新を担うため整合。複数選択の視覚クラスはDOMから手動除去。要素が無い場合のみ `olRender` フォールバック。
- 平 ↑/↓ の主経路（`vi>0`/`vi<len-1`）を `olRender` から `olFocusNodeEl` に置換。`scrollIntoView` は smooth をやめ `nearest`（瞬時）に。

#### 検証（ブラウザ実機）
- ↑/↓ で**1キー＝1行**移動（飛びなし）。ナビ中に**再描画が走らない**（要素インスタンス保持で確認）。20キー操作=約22ms（従来は毎回ノート全再構築）。コンソールエラーなし・`node --check` OK。
- 端の挙動（先頭で↑/末尾で↓）はネイティブのまま（エラーなし）。
...
検索が2系統（A:検索モーダル Ctrl+F、B:ノート内インクリメンタル検索 Ctrl+;/Ctrl+Shift+;）あり、「全データを検索してジャンプ」が重複していた。**全データ検索を検索モーダル(A)に一本化**し、ノート側の重複（全データドロップダウン）を廃止。**Ctrl+; のノート内ライブ検索（その場ハイライト）は別用途のため維持**。

#### 変更
- **Ctrl+Shift+;** → 検索モーダルを開く（旧: ノート内全データドロップダウン）。
- インクリメンタル検索バー（Ctrl+;）を**「このノート」専用**に。スコープボタンは「🌐 全データ検索」（クリックでモーダルを開く導線）に変更。`toggleIncSearchBar('all')`/`olIncSearchToggleScope` はモーダルを開くよう変更。
- 検索モーダルの結果に **B 由来の改善を移植**: 各結果に**アイコン（☐/☑/🔗/📁/•）**＋**長文の前後文脈プレビュー**（`_searchPreview`：最初のマッチ周辺に切り詰め）。
- **重複コードを削除**: `_olRenderGlobalSearchResults` / `_olSetGlobalActiveItem` / `_olGsrHover` と `olIncSearchKey`・`_olApplyIncSearchHighlight` の全データ分岐、未使用変数 `_olIncSearchScope`/`_olGlobalActiveIdx` を撤去（app.js 8586→8463行、約123行削減）。

#### 維持
- 検索モーダルのタグ絞り込み・2キーワード・検索の保存・プロジェクト名検索。
- Ctrl+; のノート内ライブハイライト検索（`_olApplyIncSearchHighlight` の現ノート経路）。

#### 検証（ブラウザ実機）
- Ctrl+; 現ノート検索のハイライト維持／スコープボタン→モーダル／Ctrl+F・Ctrl+Shift+;・🔍→モーダル／結果にアイコン＋文脈プレビュー／削除関数は undefined（参照残存なし）／コンソールエラーなし・`node --check` OK（8463行）。

---

## v1.11.1-05310835-leftscroll (2026-05-31)
### バグ修正 — キーボードで左セルへ移動時に固定列の裏に隠れる

`_scrollClearSticky`（[app.js](app.js)）は左固定列を `col-proj` のみ見ていたため、その右にある **`col-link`（同じく左sticky）の裏に隠れたセルを補正できなかった**。隠れ境界を「`col-proj` と `col-link` の右端のうち最大」に変更（thead の `th.col-proj`/`th.col-link` から算出）。これでキーボードで左の週セルへ移動した際、固定列に隠れず見える位置までスクロールする。`applyFocusToElement` 等の全呼び出し元に効く単一修正。

実機検証: 列幅 col-proj右=243 / col-link右=403 の環境で、左208pxに隠れたセルが補正後409px（link列右端をクリア）へ。コンソールエラーなし・`node --check` OK。

---

## v1.11.0-05302015-clip-p3 (2026-05-30)
### 機能追加 — コピペ Phase 3: 表（TSV/HTML表の貼付・表のコピー出力）

Excel / Google Sheets 等からの表貼付と、表ノードの外部コピーに対応。

#### 貼付
- **HTML表**（`<table>` を含むクリップボード）: Phase2 の `olParseHtmlToNodes` が表を1ノード（サニタイズ済み `<table>` を `node.html`）として取り込む（`border` 等の不要属性は除去）。
- **TSV（タブ区切り・HTML表なし）**: `olLooksLikeTsv`（過半数行にタブを要求しコードのインデント誤検出を回避）→ `olTsvToTableHtml` で先頭行ヘッダ(`th`)の `<table>` を生成し表ノード化。`olContainerPaste` にブランチ①.7を追加（リッチHTMLの後、テキスト経路②の前）。

#### コピー出力
- 表ノードをコピーすると **text/plain = TSV**（`olTableToTsv`）・**text/html = 実際の `<table>`**（`olBuildVisibleList` が表ノードを `<table>` として出力）→ Excel/Sheets へセル展開で貼り付け可能。
- 併せて通常ノードのコピー出力も、装飾 `node.html` があれば `<li>` 内にそのまま出力（太字/リンクが外部アプリで活きる）。

#### スコープ/方針
- 貼付した表は**1ノード（生html）**として保持（行展開はしない＝仕様の既定）。編集は既存の表コンテキストメニュー（`tblCtxOpen` 等）。
- TSV判定はタブのみ（CSV/カンマは誤検出回避のため対象外）。

#### 検証（ブラウザ実機）
- TSVのみ貼付→表ノード（先頭行th）／Excel風HTML表→サニタイズ表ノード（border除去）／表ノードのコピー→text/plain=TSV・html=table／TSV↔表ラウンドトリップ無損失／コード(先頭タブ)は表誤検出せず／プレーン多行はアウトライン経路維持／コンソールエラーなし・`node --check` OK（8586行）。

#### 残（次フェーズ）
- Phase 4: 画像往復強化（コピー時に画像保持・外部imgの扱い。画像はGitHub必須維持）。

---

## v1.10.1-05301940-ui-fixes (2026-05-30)
### バグ修正/調整 — 追加3課題（入力欄がたつき・PJ並べ替え・タイトル行配色）

#### ① セルの追加入力欄（qarow）のがたつき解消
- `.qarow` は従来 `max-height:0→40px` で開閉していたため、フォーカス/ホバーで高さが変わりレイアウトが揺れていた。**高さを常時確保**（`margin-top:6px`＋自然高さ）し、表示切替は `opacity`（＋`pointer-events`）のみに変更。表示タイミングは従来どおり `:focus-within` / `:has(.kfocus)` / `:hover`。実機で高さ不変（21px固定）・フォーカスで opacity 0→1 を確認。

#### ② プロジェクト並べ替えで Phase/Link が取り残される
- 原因: Phase/Link/プロジェクトノートは `dailyOutline["proj:{index}"]`（**index基準**）で保持しており、`pDrop`（並べ替え）が `S.projects` のみ並べ替えて `proj:` キーを追従させていなかった（週タスクは projTag=名前基準なので追従していた）。
- 修正: `pDrop` で `proj:{i}` ノートをプロジェクト配列と同じ順序に並べ替え。**同根の `deleteProj` も修正**（削除位置の `proj:` ノートを除去し後続を前詰め）。実機で並べ替え後も Phase/Link が当該プロジェクトに追従することを確認。

#### ③ タイトル行（プロジェクトバンド）の配色が派手すぎる
- バンド背景を `var(--pc)` 直接から **`color-mix(... 64%, #000)`（黒36%）** へ深め、`pcell-hdr` も深く（48%）。ダークモードも追従（58%/42%）。彩度を抑え深みのある色味に。週セルの淡色tint・左色帯は従来どおり。

#### 注意
- ②は既存データの `proj:` キー不整合は修正しない（今後の並べ替え/削除から正しくなる）。過去に並べ替え済みで Phase/Link がズレている場合は再度並べ替えで是正可能。
- 配色はCSS変更。デプロイ後に反映されない場合は **Ctrl+Shift+R**（ブラウザHTTPキャッシュ。SWはnetwork-firstのためオンラインなら最新取得）。
